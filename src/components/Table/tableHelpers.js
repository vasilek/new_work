...
helpers.generateHeaders = function (headers = [], datedLabors, onAccept, calendar=[]) {
    ...
    for (var i = 1; i <= headers.length; i++) {
        ...
        th[i] = (
            <th key={i} className="table-header" width="10%" style={{backgroundColor:backColor, color:fontColor}}>
                <div>
                    ...
                    <div data-tip={"Подтвердить все трудозатраты за " + headers[i - 1]}>
                        <Icon name="acceptTrud"
                              className={`clickable-image openTrud small ` + ( canAcceptAll ? "" : "noDisplay")}
                              onClick={onAccept.bind(this, headers[i - 1], newLabors)}/>
                    </div>
                </div>
            </th>
        );
...
helpers.generateRows = function (data = [], clickHandler, rowClickHandler, props) {
    ...
    for (var i = 0; i < Object.keys(elements).length; i++) {
        ...
        ${elem.active && (headers[j] === props.currentDay || props.currentDay === false) ? "active" : ''}`}
                    width={tdWidth + "%"}
        ...
        rows[i] = (
            <tr key={i}>
                <td width="30%" className={`tableCell ${elem.active ? "active" : ''}`}
                    onClick={rowClickHandler.bind(this, labors, elem.id)}>
...
helpers.generateUserReportTable = function (users, dateWords, name, position, chname, chposition, half) {
    ...
    let setHalf = false;
    let daysArr = [];
    let rownumbers = [<td className="tg-yw4l" colSpan="3" key="1">1</td>,
        <td className="tg-yw4l" colSpan="3" key="2">2</td>, <td className="tg-yw4l" colSpan="3" key="3">3</td>];
    for (let i = 0; i < daysNumber; i++) {
        ...
        rownumbers.push(<td className="tg-yw4l"
                            key={setHalf ? i + 3987 : i + 3488}>{(setHalf ? (i + 5) : (i + 4))}</td>);
    ...
    for (var ind = 0; ind < users.length; ind++) {
        ...
        for (; i < user.longestFinance; i++) {
            ...
            for (let j = 0; j < user.days.length; j++) {
                ...
                if (user.days[i].finance && user.days[j].finance[i]) {
                    firstRow.push(<td className="tg-yw4l" rowSpan="2"
                                      key={j + i + 15228}>{user.days[j].finance[i].hours}</td>)
                    secondRow.push(<td className="tg-yw4l" rowSpan="2"
                                       key={j + i + 323458}>{user.days[j].finance[i].name}</td>)
                } else {
        ...
        for (let i = 0; i < user.totalFinance.length; i++) {
            ...
            if (tick) {
                ...
                if (user.totalFinance[i]) {
                    firstRows[globalCounter].push(<td className="tg-yw4l" rowSpan="2" colSpan="3"
                                                      key={ i - 128}>{user.totalFinance[i].value}
                        - {`${user.totalFinance[i].days}/${user.totalFinance[i].hours}`}</td>);
                    firstRows[globalCounter][15] = (
                        <td className="tg-yw4l" rowSpan="2" colSpan="3" key={ i - 12338}>{user.halfFinance[i].value}
                            - {`${user.halfFinance[i].days}/${user.halfFinance[i].hours}`}</td>)
                } else {
                    break;
                }
            } else {
                tick = true;
                if (user.totalFinance[i]) {
                    secondRows[globalCounter].push(<td className="tg-yw4l" rowSpan="2" colSpan="3"
                                                       key={ i - 15228}>{user.totalFinance[i].value}
                        - {`${user.totalFinance[i].days}/${user.totalFinance[i].hours}`}</td>);
                    secondRows[globalCounter][15] = (
                        <td className="tg-yw4l" rowSpan="2" colSpan="3" key={ i - 15222348}>{user.halfFinance[i].value}
                            - {`${user.halfFinance[i].days}/${user.halfFinance[i].hours}`}</td>);
                } else {
                    break;
                }
        ...
        for (let i = 0; i < user.days.length; i++) {
            if (i + 1 === 16 && !setHalf) {
                userInfo.push(<td className="tg-yw4l" colSpan="3"
                                  key="itogo2">{`Ф - ${user.halfDays}/${user.halfHours}`}</td>);
                i--;
                setHalf = true;
                continue;
            }
        ...
        userInfo.push(<td className="tg-yw4l" colSpan="3"
                          key="itogo">{`Ф - ${user.totalDays}/${user.totalHours}`}</td>);
