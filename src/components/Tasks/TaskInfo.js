    ...
    <div style={{display: 'flex', justifyContent: "flex-end"}}>
        <div style={{marginRight: "15px", display: "flex", flexDirection: "row"}}>
            <div data-tip="Список трудозатрат">
                <Icon name="clock" className={`clickable-image clock ${activePanel === "trud" ? 'active' : ''}`}
                      onClick={chooseTrudTab}/>
            </div>
            <div data-tip="Комментарии к задаче">
                <Icon name="comment" className={`clickable-image comment ${activePanel === "comment" ? 'active' : ''}`}
                      onClick={chooseCommentTab}/>
            </div>
        </div>
...
const addTrudButtonF = (props) => ((task) => (
    <div data-tip={ task.rights.time ? "" : "Недостаточно прав"} flex="2"
         className={"addTrudButtonContainer " + (props.activeTab !== "trud" ? "noDisplay" : "")}>
        <div
...
const popoverMenu = (props, context) => {
    ...
    menuItems.push(<MenuItem key={1} onClick={props.handleAddNewSubTask.bind(context, props.task)}
                             primaryText="Cоздать подзадачу"/>)
    if (task.rights.accept) {
        menuItems.push(<MenuItem key={2} onClick={props.declineTask.bind(context, props.task)}
                                 primaryText="Отклонить задачу"/>)
    }
    if (task.rights.accept && task.rawstatus === 0) {
        menuItems.push(<MenuItem key={3} onClick={props.acceptTask.bind(context, props.task)}
                                 primaryText="Подтвердить задачу"/>)
    }
    if (task.rights.accept && task.rawstatus !== 0) {
        menuItems.push(<MenuItem key={4} onClick={props.completeTask.bind(context, props.task)}
                                 primaryText="Завершить задачу"/>)
    }
    if (task.rights.delete) {
        menuItems.push(<MenuItem key={5} onClick={props.deleteTask.bind(context, props.task)}
                                 primaryText="Удалить задачу"/>)
    }
    if (task.rights.reopen) {
        menuItems.push(<MenuItem key={7} onClick={props.reopenTask.bind(context, props.task)}
                                 primaryText="Переоткрыть задачу"/>)
    }
    menuItems.push(<MenuItem key={6} onClick={props.copyTask.bind(context, props.task)}
                             primaryText="Копировать задачу"/>);
    return (<Popover
...
//             selected={props.task.startDate ? moment(props.task.startDate) : moment(new Date())}
//             // onChange={props.onDateSelect ? props.onDateSelect :refs.sbmt.click()}
...
const TaskInfoComponent = class newTaskInfo extends React.Component {
    ...
    setExecutors(list) {
        ...
        this.props.task.executors = list;
        this.props.task.rawExecutors = list;


        const SetList =  new Set(list.map(x => x.value));
        const SetExecutorsFromForm = new Set(this.props.executorsFromForm.map(x => x.id));
        let difference = new Set(
            [...SetList].filter(x => !SetExecutorsFromForm.has(x)));
    ...
    render() {
        ...
        if (!task.id && (task.id !== 0)) {
           ...
           return (<div style={{display: "none"}}></div>);
            ...
            return (
                        <form onSubmit={handleSubmit} className={task.rights.update ? "" : "no-update"}
                              style={{display: "flex", flexDirection: "column", height: "100%"}}>
                            {popover}
                            <Container className="global-task-container" vertical={true} style={{height:"100%"}}>
                                <div className="infoHeader" flex="1">
                                    <Container style={{justifyContent: "space-between"}}>
                                        <div ref="executorsSelect" className="infoHeaderBlock"
                                             style={{display: 'flex', justifyContent: "flex-begin"}}>
                                            {/*{executorsField}*/}


                                            <div>
                                                <img className="user" onClick={this.clickHandler.bind(this)} src={human}
                                                     alt="logo" style={{marginLeft: 5}}/>
                                                <PersonModalContainer isModalOpen={this.state.isModalOpen}
                                                                      closeModal={this.closeModal.bind(this)}
                                                                      initialValue={executorsFromForm}
                                                                      setExecutors={(list) => {
                                                                          {/*<ExecutorsAsyncSelectField executors={list}/>*/}
                                                                          this.setExecutors(list);
                                                                          this.props.changeFieldValue('executors', list)
                                                                      }}/>
                                            </div >
                                            ...
                                            {/*{v != 0 ?*/}
                                                {/*<Text*/}
                                                    {/*data-tip={"<b>Исполнители</b>" + this.state.executors.map(x => '<br/>' + x.label) }*/}
                                                    {/*data-html data-iscapture="true" numberOfLines={1}*/}
                                                    {/*style={{width: 300}}>*/}
                                                    {/*{this.state.executors.map(x =>*/}
                                                        {/*<Text numberOfLines={1}> {x.label},</Text>)}*/}
                                                {/*</Text> :*/}
                                                {/*<Text*/}
                                                    {/*data-tip={"<b>Исполнители</b>" + executorsFromForm.map(x => '<br/>' + x.name) }*/}
                                                    {/*data-html data-iscapture="true" numberOfLines={1}*/}
                                                    {/*style={{width: 300}}>*/}
                                            ...
                                            <div style={{display: "flex", flexDirection: "row"}}>
                                                {/*{datePicker(props, this.refs)}*/}
                                                {/*<Icon className="user" name="calendar" onClick={this.handleEdit.bind(this)}/>*/}
                                                <Field name="startDate"
                                                       newOnChange={props.onDateSelect ? props.onDateSelect : this.handleEdit.bind(this)}
                                                       component={DPicker}/>
                                            </div>
                                        </div>
                                        <div ref="ellipsis">
                                            <ImagePanel activePanel={props.activeTab}
                                                        chooseTrudTab={props.chooseCurrentTaskTab.bind(this, 'trud')}
                                                        chooseCommentTab={props.chooseCurrentTaskTab.bind(this, 'comment')}
                                                        openPopover={this.openPopover.bind(this)}/>
                                        </div>
                                    </Container>
                                </div>
                                <Container vertical={true} flex="11" height="100%" containerStyle={{
                                    overflowY: "auto",
                                    overflowX: 'hidden',
                                    paddingTop: "25px"
                                }}>
                                    <h2 flex="1" containerStyle={headerBlockStyle}
                                        style={{margin: "5px", marginBottom: "20px"}}>
                                        <Field name="name" handleChange={this.handleDebounce.bind(this)}
                                               component={NameField}/>
                                    </h2>
                                    <Container flex="3" containerStyle={codeBlockStyle}>
                                        <Panel label="Код работ">
                                            <WorkCodeField codes={this.props.codes}
                                                           debouncedUpdate={this.handleEdit.bind(this)}
                                                           // style={{width:"100%"}}
                                            />
                                        </Panel>
                                    </Container>
                      ...
                      <Field  className="fieldValue" name="description" handleChange={this.handleDebounce.bind(this)}
                             component={DescriptionField}/>
                                    ...
                                    <div className={(props.activeTab !== "comment") ? "noDisplay" : "comment"}>
                                        <TaskCommentsTabContainer sendComment={props.sendComment.bind(this, task)}
                                                                  task={task}/>
                                    </div>
                                </Container>
                                {addTrudButton(task)}
                                <AddTrudModalContainer isModalOpen={props.isTrudModalOpen}
                                                       closeModal={props.closeModal.bind(this)}
                                                       onSubmit={props.handleTrudSubmit.bind(this, task)}
