...
import RightFinancePanelContainer from "../../containers/Admin/RightFinancePanelContainer";
...
export default class FinancesList extends React.Component {
    ...
    render() {
        ...
        // props.loadFinancesTree();
        // console.log("IN FINANCES!!!",props);
        ...
        tasksIdDict = finances.treeNormalized.byId;
        tasksDict = finances.treeNormalized.byGlobalId;
        const rightPanelContainerStyle_ = this.props.rightPanelStatus ? {} : {maxWidth: "0"};
        const rightPanelClass_ = this.props.rightPanelStatus ? "" : "right-closed";
        console.log(this.props.rightPanelStatus);
        let config = {};
        const self = this;
        config.listItemRender = (item) => {
            ...
            return (
                ...
                } : props.activateFinance.bind(this, item)} className={"single-task " +
                ` ${item.isHeader ? " header-list-row " : ""} ` + (item.active ? " active" : "") + " "}
                    ...
                    {beingEdited ?
                        ...
                        : <div data-tip="Редактировать" className="pencil-edit"><Icon name="edit"
                                                                                      className={`clickable-image clock ${item.isHeader ? "non-visible" : ""}`}
        ...
        return (
            <Container>
                ...
                <ConfirmModalContainer containerStyle={{maxHeight: '0', maxWidth: '0'}}
                                       isModalOpen={this.state.isModalOpen} message={this.state.message}
                                       answer={this.state.currentQuestion.bind(this)}/>
                <div className={`splitter ${(this.props.rightPanelStatus ? "" : "noDisplay")}`}/>
                <RightFinancePanelContainer containerStyle={rightPanelContainerStyle_} className={rightPanelClass_}/>
            </Container>
        );
    }
}